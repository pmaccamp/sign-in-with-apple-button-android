...
interface SignInWithAppleCallback {
    ...
    fun onSignInWithAppleSuccess(authorizationCode: String, state: String)
...
internal fun SignInWithAppleCallback.toFunction(): (SignInWithAppleResult) -> Unit =
    { result ->
        when (result) {
            is SignInWithAppleResult.Success -> onSignInWithAppleSuccess(result.authorizationCode, result.state)
